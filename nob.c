...
bool build_and_run_o_file(Nob_Procs *procs, Nob_Cmd *cmd, const char *output,
                          const char *input) {
...
bool make_o_files_from_dir(const char *dir, Nob_Procs *procs, Nob_Cmd *cmd,
                           Nob_String_View *sv, Nob_File_Paths *nfp) {
  if (!nob_read_entire_dir(dir, nfp))
    return false;
  nob_da_foreach(const char *, it, nfp) {
    ...
    *sv = nob_sv_from_cstr(*it);
    if (**it != '.' && nob_sv_end_with(*sv, ".cpp")) {
      ...
      const char *output =
          nob_temp_sprintf("build/nob/" SV_Fmt ".o", SV_Arg(*sv));
      if (nob_needs_rebuild1(output, input)) {
        if (!build_and_run_o_file(procs, cmd, output, input))
          return false;
      }
    }
  }
...
int main(int argc, char **argv) {
  ...
  if (!make_o_files_from_dir("src/Source_files", &procs, &cmd, &sv, &nfp))
    return 1;
  if (!make_o_files_from_dir("src/Mappers", &procs, &cmd, &sv, &nfp))
    return 1;
  if (!make_o_files_from_dir("src", &procs, &cmd, &sv, &nfp))
    return 1;
