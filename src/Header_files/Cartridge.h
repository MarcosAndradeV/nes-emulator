...
public:
    ...
    std::shared_ptr<Mapper> GetMapper() { return pMapper; }  // Retorna o mapeador de memória

private:
    std::vector<uint8_t> vPRGMemory;  // Memória ROM do programa (código do jogo)
    std::vector<uint8_t> vCHRMemory;  // Memória ROM de caracteres (gráficos)
    std::shared_ptr<Mapper> pMapper;  // Gerenciador de mapeamento de memória
